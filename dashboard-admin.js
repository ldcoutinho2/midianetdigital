...
async function savePackage(k){const x=get(k);if(!x)return;try{await savePayload({...x,chave:k,nome:$('pn_'+k).value,qtd:$('pq_'+k).value,id:$('pi_'+k).value.trim(),preco:Math.round(Number($('pp_'+k).value||0)*100),custo:Number($('pc_'+k).value||0),bonusCurtidas:Number($('pbL_'+k).value||0),bonusVisualizacoes:Number($('pbV_'+k).value||0),descricao:$('pd_'+k).value,destaque:$('ph_'+k).checked,ativo:$('pa_'+k).checked,excluido:false})}catch(e){toast(e.message)}}
...
async function createPackage(){
 ...
 const x={chave:slug+'__'+pl,servico:slug,plano:pl,qtd:$('nqty').value.trim()||pl,nome:$('nname').value.trim(),id:$('nid').value.trim(),custo:Number($('ncost').value||0),preco:Math.round(Number($('nprice').value||0)*100),icone:$('nicon').value.trim()||'📦',descricao:$('ndesc').value.trim(),por:$('npor').value.trim(),bonusCurtidas:Number($('nlikes').value||0),bonusVisualizacoes:Number($('nviews').value||0),excluido:false,destaque:$('nhot').checked,ativo:$('nactive').checked,ordem:(()=>{const ex=Object.values(cfg.servicos||{}).filter(y=>y.servico===slug);return ex.length?Math.max(...ex.map(y=>Number(y.ordem||0)))+1:Object.keys(cfg.servicos||{}).length+1})()};
 if(!slug||!pl||!x.nome||!x.id||x.preco<=0)return toast('Preencha slug, nome, pacote, ID e preço');
 try{await savePayload(x);['nslug','nname','nplan','nqty','nid','ncost','nprice','ndesc','npor','nlikes','nviews'].forEach(i=>$(i).value='');$('nicon').value='📦';$('nhot').checked=false;$('nactive').checked=true;$('newBox').style.display='none'}catch(e){toast(e.message)}
}
...
function renderPlans(){const vals=[...new Set(Object.values(cfg.servicos||{}).filter(x=>x.excluido!==true).map(x=>x.servico))].sort();$('planService').innerHTML='<option value="all">Todos os serviços</option>'+vals.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('');const a=filtered('planSearch','planService').sort((x,y)=>(x.servico+x.plano).localeCompare(y.servico+y.plano));$('plansTable').innerHTML=a.map(x=>{const k=esc(x.chave);return `<tr><td><b>${esc(x.nome||x.servico)}</b><br><small class="muted">${esc(x.servico)}</small></td><td>${esc(x.plano)}</td><td><input class="input" id="tq_${k}" value="${esc(x.qtd||x.plano)}"></td><td><input class="input" id="tp_${k}" type="number" step=".01" value="${(Number(x.preco||0)/100).toFixed(2)}"></td><td><input class="input" id="tc_${k}" type="number" step=".01" value="${Number(x.custo||0)}"></td><td><input class="input" id="tl_${k}" type="number" min="0" value="${Number(x.bonusCurtidas||0)}"></td><td><input class="input" id="tv_${k}" type="number" min="0" value="${Number(x.bonusVisualizacoes||0)}"></td><td><input id="th_${k}" type="checkbox" ${x.destaque?'checked':''}></td><td><span class="pill ${x.ativo===false?'off':''}">${x.ativo===false?'Off':'Ativo'}</span></td><td><button class="btn primary" onclick="savePlan('${k}')">Salvar</button></td></tr>`}).join('')||'<tr><td colspan="8"><div class="empty">Nenhum plano encontrado.</div></td></tr>'}
window.renderPlans=renderPlans;
async function savePlan(k){const x=get(k);try{await savePayload({...x,qtd:$('tq_'+k).value,preco:Math.round(Number($('tp_'+k).value||0)*100),custo:Number($('tc_'+k).value||0),bonusCurtidas:Number($('tl_'+k).value||0),bonusVisualizacoes:Number($('tv_'+k).value||0),destaque:$('th_'+k).checked})}catch(e){toast(e.message)}}window.savePlan=savePlan;
