...
async function enviarComponenteSMM({servico, quantidade, links, nome}) {
  if (!quantidade || quantidade <= 0) return null;
  const smmId = SERVICO_MAP[`${servico}__${String(quantidade)}`] || SERVICO_MAP[`${servico}__500`];
...
async function enviarComboSMM(pedido) {
  ...
  // Seguidores: divide o total conforme a variação escolhida.
 ...
 * essa função para o pedido principal.
 */
...
app.get('/instagram/posts', async (req, res) => {
  try {
    ...
    if (!user) return res.status(400).json({ success:false, posts:[], error:'Usuário inválido' });

    const normalizePosts = (items) => (items || [])
      .map(post => ({
        url: post.url || post.postUrl || (post.shortCode || post.shortcode ? `https://www.instagram.com/p/${post.shortCode || post.shortcode}/` : ''),
        thumb: post.displayUrl || post.imageUrl || post.thumbnailUrl || post.image || post.videoUrl || '',
        caption: post.caption || '',
        tipo: post.type || post.productType || post.mediaType || ''
      }))
    ...
    try {
      ...
      items = normalizePosts(run.data);
    } catch (primaryErr) {
      console.error('[INSTAGRAM POSTS] ator principal falhou:', primaryErr.response?.data || primaryErr.message);
    }

    if (!items.length) {
      try {
        const runFallback = await axios.post(
        ...
        items = normalizePosts(runFallback.data);
      } catch (fallbackErr) {
        console.error('[INSTAGRAM POSTS] fallback falhou:', fallbackErr.response?.data || fallbackErr.message);
      }
    }

    return res.json({ success: items.length > 0, posts: items, error: items.length ? null : 'Nenhuma publicação encontrada' });
  } catch (err) {
    console.error('[ERRO instagram posts]', err.response?.data || err.message);
    return res.status(500).json({ success:false, posts:[], error:'Erro ao buscar publicações' });
  }
});
...
app.post('/criar-pedido', async (req, res) => {
  try {
    ...
    const valorTotalCentavos = valorBaseCentavos;
...
console.log(JSON.stringify(payment, null, 2));
  ...
  'pix',
  `${nome} | ${telefone} | Perfil: ${instagram} | ${servico} ${plano}`,
  valorReais
);
...
const smmData = pedido.combo ? await enviarComboSMM(pedido) : await enviarPedidoSMM(pedido);
   ...
   await registrarEvento(
  'venda',
  `${pedido.nome} | ${pedido.telefone} | Perfil: ${pedido.instagram} | ${pedido.servico} ${pedido.plano}`,
  Number((pedido.valor / 100).toFixed(2))
);
    await enviarPurchaseMeta(pedido);

    console.log(`[SUCESSO PUSHINPAY] Pedido ${pedido.id} concluido. SMM: ${pedido.combo ? 'combo' : smmData.order}`);
...
const smmData = pedido.combo ? await enviarComboSMM(pedido) : await enviarPedidoSMM(pedido);
    ...
    console.log(`[SUCESSO] Pedido ${pedidoId} concluido. SMM: ${pedido.combo ? 'combo' : smmData.order}`);
