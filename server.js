...
async function enviarPedidoSMM(pedido) {
  const dist = pedido.distribuicao;
  const pubDist = Array.isArray(dist?.publicacoes)
    ? dist.publicacoes.filter(x => x && String(x.link || '').trim())
    : [];

  // Publication services can carry an explicit quantity for every selected post.
  // This preserves the customer's manual distribution instead of re-dividing equally.
  if (pubDist.length && ['curtidas-brasileiras','visualizacoes','comentarios'].includes(pedido.servico)) {
    const resultados = [];
    for (const item of pubDist) {
      const link = String(item.link).trim();
      let quantidade = 0;
      if (pedido.servico === 'curtidas-brasileiras') quantidade = Number(item.curtidas || 0);
      if (pedido.servico === 'visualizacoes') quantidade = Number(item.visualizacoes || 0);
      if (pedido.servico === 'comentarios') quantidade = Number(item.comentarios || 0);
      ...
      resultados.push({
        ...
        quantity: quantidade,
        link,
        order: smmResp.data?.order || null
      });
    }

    return {
      multiplos: resultados.length > 1,
      quantidadeTotal: resultados.reduce((a,x)=>a+Number(x.quantity||0),0),
      totalLinks: resultados.length,
      pedidos: resultados,
      order: resultados.map(r=>r.order).filter(Boolean).join(', ')
    };
  }

  // Legacy/general behavior.
  let links = [];
  try {
  ...
  if (!links.length) throw new Error('Nenhum link válido para enviar ao SMM');
...
app.post('/criar-pedido', async (req, res) => {
  try {
    ...
    if (!combo && ['curtidas-brasileiras','visualizacoes','comentarios'].includes(servico)) {
      const dist = req.body.distribuicao || {};
      const pubs = Array.isArray(dist.publicacoes) ? dist.publicacoes : [];
      const total = pubs.reduce((a,x) => {
        if (servico === 'curtidas-brasileiras') return a + Number(x.curtidas || 0);
        if (servico === 'visualizacoes') return a + Number(x.visualizacoes || 0);
        return a + Number(x.comentarios || 0);
      }, 0);
      if (!pubs.length || total !== Number(plano)) {
        return res.status(400).json({ error: 'A distribuicao das publicacoes nao confere com a quantidade do plano' });
      }
    }
