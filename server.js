...
async function salvarConfiguracaoServico(item) {
  ...
  const payload = {
    chave, servico: String(item.servico || ''), plano: String(item.plano || ''), id: String(item.id || ''),
    preco: Math.round(Number(item.preco || 0)), custo: Number(item.custo || 0), ativo: item.ativo !== false,
    nome: String(item.nome || SERVICO_TITULOS[item.servico] || item.servico || ''), qtd: String(item.qtd || item.plano || '')
  };
...
app.get('/config/public',async(req,res)=>{try{const config=await buscarConfiguracao();const servico=String(req.query.servico||'');const itens=Object.values(config.servicos||{}).filter(x=>!servico||x.servico===servico);return res.json({servicos:itens.map(x=>({chave:x.chave||`${x.servico}__${x.plano}`,servico:x.servico,plano:x.plano,preco:Number(x.preco||0)/100,ativo:x.ativo!==false,nome:x.nome||SERVICO_TITULOS[x.servico]||x.servico,qtd:x.qtd||x.plano})),titulos:SERVICO_TITULOS});}catch{res.status(500).json({error:'Erro ao carregar configuração pública'});}});
...
<div class="section" id="admin-config"><h2>⚙️ Gerenciar site e financeiro</h2><div class="small" style="margin-bottom:14px">Altere preços, custos, IDs SMM e disponibilidade dos pacotes sem mexer no código. As alterações ficam salvas no Supabase.</div><div id="adminStatus" class="small" style="margin-bottom:10px"></div><div class="admin-table" id="servicosAdmin"></div><div style="margin-top:14px"><h3 style="font-size:15px;margin-bottom:10px">📢 Adicionar investimento em anúncio</h3><div class="filters"><div><label>Data</label><input type="date" id="adsData"></div><div><label>Plataforma</label><input id="adsPlataforma" value="Meta Ads"></div><div><label>Campanha</label><input id="adsCampanha" placeholder="Ex.: Seguidores"></div><div><label>Valor</label><input type="number" id="adsValor" min="0" step="0.01" placeholder="0,00"></div><button onclick="salvarAds()">💾 Salvar anúncio</button></div><div id="adsLista" style="margin-top:10px"></div></div></div>
...
function paginar(a,p){return a.slice(p*porPagina,p*porPagina+porPagina)}function botoesPaginacao(total,p,tipo,el){const n=Math.ceil(total/porPagina)||1;document.getElementById(el).innerHTML='<button onclick="mudarPagina(\\''+tipo+'\\',-1)">← Anterior</button><span>Página '+(p+1)+' de '+n+'</span><button onclick="mudarPagina(\\''+tipo+'\\',1)">Próximos 10 →</button>'}
...
function renderDias(a,p){const it=paginar(a,p);document.getElementById('dias').innerHTML=it.length?it.map(x=>'<div class="row"><span>'+x[0]+'</span><strong>Pix: '+x[1].pix+' | Vendas: '+x[1].vendas+' | Fat: '+formatBR(x[1].faturamento)+'</strong></div>').join(''):'<div class="row"><span>Nenhum dado</span><strong>-</strong></div>';botoesPaginacao(a.length,p,'dias','pagDias')}
...
async function carregarAdmin(){try{const r=await adminFetch('/admin/config');if(!r.ok)throw new Error('Não autorizado');adminConfig=await r.json();renderAdmin()}catch(e){document.getElementById('adminStatus').textContent='❌ '+e.message}}
function renderAdmin(){const el=document.getElementById('servicosAdmin');if(!el||!adminConfig)return;const itens=Object.values(adminConfig.servicos||{}).sort((a,b)=>(a.servico+a.plano).localeCompare(b.servico+b.plano));el.innerHTML=itens.map(x=>{const chave=x.chave||x.servico+'__'+x.plano,esc=chave.replace(/[^a-zA-Z0-9_-]/g,'_');return '<div class="admin-row"><span><b>'+x.servico+'</b><br><small>Pacote '+x.plano+'</small></span><input id="id_'+esc+'" value="'+(x.id||'')+'" placeholder="ID SMM"><input id="custo_'+esc+'" type="number" step="0.01" value="'+Number(x.custo||0)+'" placeholder="Custo"><input id="preco_'+esc+'" type="number" step="0.01" value="'+(Number(x.preco||0)/100).toFixed(2)+'" placeholder="Venda"><label style="font-size:12px"><input id="ativo_'+esc+'" type="checkbox" '+(x.ativo!==false?'checked':'')+'> Ativo</label><button onclick="salvarServico('"+chave+"')">Salvar</button></div>'}).join('');renderAds()}
async function salvarServico(chave){const x=adminConfig.servicos[chave],esc=chave.replace(/[^a-zA-Z0-9_-]/g,'_');const id=document.getElementById('id_'+esc).value.trim(),custo=Number(document.getElementById('custo_'+esc).value||0),preco=Number(document.getElementById('preco_'+esc).value||0),ativo=document.getElementById('ativo_'+esc).checked;const r=await adminFetch('/admin/config/servico',{method:'POST',body:JSON.stringify({...x,chave,id,custo,preco:Math.round(preco*100),ativo})}),j=await r.json();document.getElementById('adminStatus').textContent=j.ok?'✅ Salvo: '+chave:'❌ '+(j.error||'Erro');if(j.ok){adminConfig.servicos[chave]=j.item;renderAdmin();carregar()}}
function renderAds(){const el=document.getElementById('adsLista');if(!el)return;const ads=adminConfig?.anuncios||[];el.innerHTML=ads.slice().sort((a,b)=>String(b.data).localeCompare(String(a.data))).map(a=>'<div class="row"><span>'+a.data+' • '+a.plataforma+' • '+(a.campanha||'Sem campanha')+'</span><strong>'+Number(a.valor||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'})+'</strong></div>').join('')||'<div class="small">Nenhum investimento cadastrado.</div>'}
