...
function formatarDetalhesPedido({nome, telefone, instagram, servico, plano, distribuicao, bump, bump_publicacao}) {
  const linhas = [
    `${nome} | ${telefone}`,
    `Perfil: ${instagram}`,
    `Pacote: ${servico} ${plano}`
  ];

  const pubs = Array.isArray(distribuicao?.publicacoes) ? distribuicao.publicacoes : [];
  const totalCurtidas = Number(distribuicao?.totalCurtidas || 0);
  const totalVisualizacoes = Number(distribuicao?.totalVisualizacoes || 0);

  if (pubs.length) {
    if (totalCurtidas > 0) {
      linhas.push(`❤️ Curtidas: ${totalCurtidas}`);
    }
    if (totalVisualizacoes > 0) {
      linhas.push(`👁️ Visualizações: ${totalVisualizacoes}`);
    }
    pubs.forEach((pub, i) => {
      ...
      if (!link) return;
      const partes = [`📸 Publicação ${i + 1}: ${link}`];
      if (likes > 0) partes.push(`❤️ ${likes} curtidas`);
      if (views > 0) partes.push(`👁️ ${views} visualizações`);
      linhas.push(partes.join(' | '));
    });
  }

  if (bump && bump_publicacao) {
    linhas.push(`Bump: 500 curtidas | Publicações: ${bump_publicacao}`);
  }
...
function montarDashboard(eventos) {
  ...
  eventos.forEach(e => {
    ...
    const dia = data.toLocaleDateString('pt-BR', tz);
...
app.post('/criar-pedido', async (req, res) => {
  try {
    ...
    if (combo) {
      ...
      const likesNeed = [1000,2000,3000,4000,5000].includes(Number(plano)) || Number(plano) === 1500;
      const totalLikes = pubs.reduce((a,x)=>a+Number(x.curtidas||0),0);
      const totalViews = pubs.reduce((a,x)=>a+Number(x.visualizacoes||0),0);
      const expectedLikes = Number(dist.totalCurtidas||0);
      const expectedViews = Number(dist.totalVisualizacoes||0);
      if ((likesNeed || expectedLikes > 0 || expectedViews > 0) && !pubs.length) {
      ...
      if (expectedLikes !== totalLikes || expectedViews !== totalViews) {
        return res.status(400).json({ error: 'A distribuicao de curtidas/visualizacoes nao confere com o pacote' });
      }
      // Regra de segurança: toda publicação que receber curtidas/visualizações
      // precisa receber pelo menos 150. Não há limite máximo de publicações;
      // a própria soma do pacote determina quantas cabem com esse mínimo.
      for (const pub of pubs) {
        if (Number(combo?.bonus?.likes) && Number(pub.curtidas || 0) < 150) {
          return res.status(400).json({ error: 'Cada publicacao deve receber no minimo 150 curtidas' });
        }
        if (Number(combo?.bonus?.views) && Number(pub.visualizacoes || 0) < 150) {
          return res.status(400).json({ error: 'Cada publicacao deve receber no minimo 150 visualizacoes' });
        }
      }
    }

    if (!combo && (servico === 'curtidas-brasileiras' || servico === 'visualizacoes')) {
      let linksDiretos = [];
      try {
        linksDiretos = JSON.parse(instagram);
        if (!Array.isArray(linksDiretos)) linksDiretos = [instagram];
      } catch (e) {
        linksDiretos = [instagram];
      }
      linksDiretos = linksDiretos.filter(x => x && String(x).trim());
      if (!linksDiretos.length) return res.status(400).json({ error: 'Selecione ao menos uma publicacao' });
      const qtd = Number(plano);
      const minimoPorPublicacao = Math.floor(qtd / linksDiretos.length);
      if (minimoPorPublicacao < 150) {
        return res.status(400).json({ error: 'Cada publicacao deve receber no minimo 150' });
      }
    }
...
console.log(JSON.stringify(payment, null, 2));
    ...
    await registrarEvento(
      'pix',
      formatarDetalhesPedido({ nome, telefone, instagram, servico, plano, distribuicao: req.body.distribuicao, bump: bumpAtivo, bump_publicacao }),
      valorReais
    );
