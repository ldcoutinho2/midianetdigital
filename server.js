...
async function enviarComboSMM(pedido) {
  ...
  for (const item of distLikes) result.curtidas.push(...(await enviarComponenteSMM({ servico: pedido.bonusCurtidasTipo || 'curtidas-brasileiras', quantidade: item.quantidade, links: [item.link], nome: item.link, smmId: pedido.bonusCurtidasId || undefined }) || []));
  for (const item of distViews) result.visualizacoes.push(...(await enviarComponenteSMM({ servico: 'visualizacoes', quantidade: item.quantidade, links: [item.link], nome: item.link, smmId: pedido.bonusVisualizacoesId || undefined }) || []));
  return result;
}
...
async function salvarConfiguracaoServico(item) {
  ...
  const payload = {
    ...
    bonusCurtidas: Number(item.bonusCurtidas || 0), bonusCurtidasTipo: String(item.bonusCurtidasTipo || 'curtidas-brasileiras'), bonusCurtidasId: String(item.bonusCurtidasId || '').trim(), bonusVisualizacoes: Number(item.bonusVisualizacoes || 0), bonusVisualizacoesId: String(item.bonusVisualizacoesId || '').trim(), excluido: item.excluido === true
  };
...
function montarDashboard(eventos, config = montarConfiguracaoPadrao(), filtros = {}) {
  ...
  for(const pedido of pedidosPagos.values()){
    ...
    const custo=Number(pedido.custoServico ?? config.servicos?.[chave]?.custo ?? 0);
...
app.get('/config/public',async(req,res)=>{try{
  ...
  return res.json({servicos:itens.map(x=>({
    ...
    descricao:x.descricao||'',icone:x.icone||'📦',destaque:x.destaque===true,por:x.por||'',ordem:Number(x.ordem||100),bonusCurtidas:Number(x.bonusCurtidas||0),bonusCurtidasTipo:String(x.bonusCurtidasTipo||'curtidas-brasileiras'),bonusCurtidasId:String(x.bonusCurtidasId||''),bonusVisualizacoes:Number(x.bonusVisualizacoes||0),bonusVisualizacoesId:String(x.bonusVisualizacoesId||''),excluido:x.excluido===true
...
async function enviarBonusConfigurado(pedido){
  ...
  const likeId=pedido.bonusCurtidasId||config.servicos?.['curtidas-brasileiras__500']?.id||SERVICO_MAP['curtidas-brasileiras__500'];
  const likeServico=pedido.bonusCurtidasTipo||'curtidas-brasileiras';
  const viewId=pedido.bonusVisualizacoesId||config.servicos?.['visualizacoes__500']?.id||SERVICO_MAP['visualizacoes__500'];
  const out={likes:0,views:0,orders:[]};
  for(const p of pubs){const link=String(p.link||'').trim();if(!link)continue;if(likes>0&&likeId){const r=await enviarComponenteSMM({servico:likeServico,quantidade:likes,links:[link],nome:link,smmId:likeId});out.likes+=likes;out.orders.push(...(r||[]));}if(views>0&&viewId){const r=await enviarComponenteSMM({servico:'visualizacoes',quantidade:views,links:[link],nome:link,smmId:viewId});out.views+=views;out.orders.push(...(r||[]));}}
  return out;
}
